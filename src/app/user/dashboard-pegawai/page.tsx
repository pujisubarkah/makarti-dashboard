...
const DashboardPegawaiPage: React.FC = () => {
  ...
  const golonganOptions = Array.from(new Set(pegawaiList.map((p) => p.golongan).filter(Boolean)));
  
  let filteredPegawai = pegawaiList;
  if (unitFilter) {
    filteredPegawai = filteredPegawai.filter((pegawai) => pegawai.users_pegawai_unit_kerja_idTousers?.unit_kerja === unitFilter);
  }
  if (golonganFilter) {
    filteredPegawai = filteredPegawai.filter((pegawai) => pegawai.golongan === golonganFilter);
  }
